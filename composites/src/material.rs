...
impl Material {
    ...
    pub fn Isotropic(em: f64, nu: f64, density: Option<f64>) -> Material {
        ...
        Material {
            matrix: MechanicalMatrix::Compliance(compliance),
            density: density,
        }
    }

    #[allow(non_snake_case)]
    pub fn TransverselyIsotropic(
        em_ll: f64,
        em_tt: f64,
        nu_lt: f64,
        g_lt: f64,
    ...
    pub fn get_compliance(&self) -> na::SMatrix<f64, 6, 6> {
        match &self.matrix {
            ...
            MechanicalMatrix::Stiffness(matrix) => matrix
                .try_inverse()
                .expect("Stiffness matrix must be invertible"),
        }
    }
    pub fn get_stiffness(&self) -> na::SMatrix<f64, 6, 6> {
        match &self.matrix {
            ...
            MechanicalMatrix::Compliance(matrix) => matrix
                .try_inverse()
                .expect("Compliance matrix must be invertible"),
        }
    }
    ...
    pub fn get_plane_strain_stiffness(&self) -> na::SMatrix<f64, 3, 3> {
        ...
        submatrix
            .try_inverse()
            .expect("Submatrix must be invertible")
    }
}
...
mod tests {
    ...
    fn test_isotropic_material_compliance() {
        ...
        assert_eq!(
            (compliance[(0, 0)] * 10f64.powi(NDIGITS)) as i64,
            (1.0 / em * 10f64.powi(NDIGITS)) as i64
        );
        assert_eq!(
            (compliance[(0, 1)] * 10f64.powi(NDIGITS)) as i64,
            (-nu / em * 10f64.powi(NDIGITS)) as i64
        );
        assert_eq!(
            (compliance[(3, 3)] * 10f64.powi(NDIGITS)) as i64,
            (2.0 * (1.0 + nu) / em * 10f64.powi(NDIGITS)) as i64
        );
    }
