...
fn get_config<'a>(
    no: usize,
    sets: &'a HashMap<ComponentType, Vec<Component>>,
    sizes: &'a Vec<usize>,
) -> HashMap<ComponentType, &'a Component> {
    ...
    W: DataWriter,
{
    pub fn new<'a>(
        ...
        for required_ctype in composer.require_component_types.iter() {
            if !requested_ctypes.contains(required_ctype) {
                ...
                return Err(format!(
                    "Composer required component type: '{}', which is not required by evaluations",
                    required_ctype
                ));
            }
        }
    ...
    fn query_sets(
        &self,
        queries: HashMap<ComponentType, SQLQuery>,
    ) -> HashMap<ComponentType, Vec<Component>> {
    ...
    fn evaluate(&mut self, subcomponent_sets: HashMap<ComponentType, Vec<Component>>) {
        ...
        for i in 0..total_combinations {
            ...
            let subcomponents: Vec<(String, String)> = config
                .iter()
                .map(|(ctype, &comp)| (ctype.clone(), comp.id.clone()))
                .collect();
            let existing = self
                .dataset
                .find(&select_component_with_subcomponents(&subcomponents))
                .unwrap();
            if existing.len() != 0 {
            ...
            if compostions_exits {
                self.write_to
                    .write_component(&component, &WriteModus::Update)
                    .expect("Updating Component should not be an issue!");
            } else {
                self.write_to
                    .write_component(&component, &WriteModus::Insert)
                    .expect("Writing new Component should not be an issue!");
            }
        }
    }
    ...
    pub fn evaluate_all(&mut self) {
        let subcomponent_sets: HashMap<ComponentType, Vec<Component>> =
            self.query_sets(self.create_minimal_queries());
        self.evaluate(subcomponent_sets);
    }
}
