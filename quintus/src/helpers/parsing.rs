...
pub fn parse_tol_value(txt_with_optional_tolerance: &str) -> Result<(f64, Tolerance), ()>{
    if txt_with_optional_tolerance.contains("+/-"){
        ...
        return Ok((value, Tolerance { max: tol.clone(), min: -1.0 * tol }));
    } else {
...
mod tests {
    ...
    fn parse_values() {
        let test_cases = vec![
            ("3.2312 (+/-23.2312)", (3.2312, Tolerance{min: -23.2312, max: 23.2312})),
            ("42 (+/- 2)", (42.0, Tolerance{min: -2.0, max: 2.0})),
            ("32233.2312", (32233.2312, Tolerance{min: 0.0, max: 0.0})),
            ("110(+/-0.8)", (110.0, Tolerance{min: -0.8, max: 0.8}))
        ];
