...
use crate::structures::{Component, CompositionType, Measurement, Source, SourceType, Tolerance, Unit};
...
impl SQLiteData{
    pub fn new(path: &str) -> Result<SQLiteData> {
        ...
        let sqlite = SQLiteData{conn: Arc::new(Mutex::new(conn)), idx_view: String::from("all_components")};
        if is_new {
    ...
    fn create_view(&self, filter: Option<String>){
        ...
        let sql: String;
        if filter.is_none() {
            sql = format!("CREATE TEMP VIEW IF NOT EXISTS {} (id) AS
            SELECT
            id    STRING
            FROM components;", self.idx_view);
        } else {
            sql = format!("CREATE TEMP VIEW IF NOT EXISTS {} (id) AS
            SELECT
            id    STRING
            FROM components
            WHERE components.id IN ({});", self.idx_view, filter.unwrap());
        }
        match conn.execute(
            sql.as_str(),
...
impl DataReader for SQLiteData{
    fn components(&self) -> Vec<Component>{
        ...
        let mut stmt_tags = conn.prepare("SELECT tag FROM tags WHERE component_id = :id").unwrap();
        ...
        let mut stmt_composition = conn.prepare("SELECT component_type, component_id FROM compositions WHERE composition_id = :id").unwrap();
...
impl DataWriter for SQLiteData {
    ...
    fn clean_up(&self){
        let con = self.conn.lock().expect("Get a Lock of the mutex should work!");
        ...
        con.execute("DELETE FROM tags where component_id NOT IN (SELECT components.id from components)", []).expect("Should not fail!");
        con.execute("VACUUM", []).expect("Should not fail!");
    }

    fn delete_component(&mut self, component: &Component) {
        let mut conn = self.conn.lock().unwrap();
        let tx = conn.transaction().expect("Starting a transaction should normaly not fail");
        tx.execute(&"DELETE FROM compositions WHERE composition_id = ?1 OR component_id = ?1", (&component.id,)).expect("Should not fail!");
        tx.execute(&"DELETE FROM tags WHERE component_id = ?1", (&component.id,)).expect("Should not fail!");
        tx.execute(&"DELETE FROM measurements WHERE component_id = ?1 OR component_id = ?1", (&component.id,)).expect("Should not fail!");
        tx.execute(&"DELETE FROM components WHERE id = ?1", (&component.id,)).expect("Should not fail!");
        tx.commit().expect("Should not fail!");
    }

    fn write_component(&mut self, component: &Component, modus: &WriteModus) -> Result<(), impl std::error::Error> {
        ...
        match tx.execute(&stmt_component, 
            rusqlite::params![&component.id, &component.name, &component.description, composition_type]
        ){
            Ok(_) => {
...

        {
            if matches!(modus, WriteModus::Update) {
                tx.execute("DELETE FROM tags WHERE component_id = ?1", params![&component_id]).expect("Deleting should not cause an error!");
            }
            let mut stmt= tx.prepare("INSERT INTO tags (component_id, tag) VALUES (?1, ?2)").expect("Should not fail if setup correct!");
        ...
        {
            if matches!(modus, WriteModus::Update) {
                tx.execute("DELETE FROM measurements WHERE component_id = ?1", params![&component_id]).expect("Deleting should not cause an error!");
            }
            ...
            for (property_name, measurement) in &component.properties{
                ...
                match stmt.execute(
                    rusqlite::params![&measurement.id, &component_id, &measurement.name, &measurement.value, &measurement.unit.unit, &measurement.tolerance.min, &measurement.tolerance.max, &source_type, &source_remark,conditionof]
                ){
                    Ok(_) => {
        ...
        {
            if matches!(modus, WriteModus::Update) {
                tx.execute("DELETE FROM compositions WHERE composition_id = ?1", params![&component_id]).expect("Deleting should not cause an error!");
            }
            if component.composition.is_some(){
                ...
                let Some(composition) = &component.composition else {panic!("Should not reach here!")};
                for (component_type, id) in composition {
                    stmt.execute((component_id.to_string(), id.to_string(), component_type)).unwrap();
                }
...
impl DataSet for SQLiteData{
    ...
    fn reduce(&self, filter: &str) -> Self::Output {
        ...
        let idx_view = String::from(self.idx_view.as_str()) + "_" + hex::encode(filter_hash).as_str();
        let sqlite = SQLiteData{conn: Arc::clone(&self.conn), idx_view: idx_view};
...
mod tests {
    ...
    fn insert_component() -> Result<(),std::fmt::Error> {
        ...
        assert_eq!(dataset.write_component(&component, &WriteModus::Insert).is_ok(), true);
    ...
    fn filter_tags() -> Result<(),std::fmt::Error> {
        ...
        let should_be_equal_taged_two =dataset.find("SELECT component_id FROM tags WHERE tag = 'two'");
