    ...
    fn from_sheet(sheet: &Worksheet, transformer: &NameTransformers) -> SourceColorCoding{
        ...
        for row_idx in 1..=sheet.get_highest_row(){
            ...
            if let Some(color) = cell.get_style().get_background_color() { 
                source_color = ColorString{argb: color.get_argb().to_string()};
            } 
            else {
                break
            }
            info!(target: "excel_events", "{}: '{}'", source_color.argb, source_value);
            result.color_coding.insert(source_color, transformer.get_source(&source_value));
        }
        return result
    }
}
...
fn is_valid_cell_value(value: & str)-> bool{
    return value != "" && value != "." && value !="-" && value != "#DIV/0!" && value!= "#VALUE!" && !value.is_empty()
}
...
impl ExcelData {
    ...
    fn read_sheet(&self, sheet: &Worksheet, config: &ConfigSheet) -> Vec<Component>{
        ...
        let unit: Vec<Unit> = self.read_row(sheet, pointers.units.clone()).into_iter().map(|txt| Unit::from(&txt).unwrap()).collect();
            ...
            for col_idx in 0..=sheet.get_highest_column()-1{
                ...
                if !prefix.is_empty() {
                    if !prefix.starts_with('{'){
                        {
                            let component = components_lookup.get_mut::<String>(&state.main_component_id).unwrap();
                            if component.composition.is_none(){
                                component.composition = Some(HashMap::new());
                            }
                            if component.composition.as_ref().unwrap().get(prefix).is_none(){
                                let sub_component = Component::new();
                                component.composition.as_mut().unwrap().insert(prefix.clone(), sub_component.id.clone());
                                components_lookup.insert(sub_component.id.clone(), sub_component);
                            }
                        }
                        let component = components_lookup.get::<String>(&state.main_component_id).unwrap();
                        state.child_component_id = Some(component.composition.as_ref().unwrap().get(prefix).unwrap().clone());
                    } else {
                ...
                match &name as &str {
                    "name" | "material" => {
                        let component = components_lookup.get_mut::<String>(state.get_current_component_id()).unwrap();
                    ...
                    "sources" | "comment" => {
                        ...
                        for (_, property) in &mut component.properties{
                            if property.source.is_some(){
                                ...
                                source.remark = source.remark.clone() + &cell.get_value().to_string() + "\n";
                            } else {
                    ...
                    "attributes" => {
                        ...
                        let attributes: Vec<HashMap<String, Attribute>>= match serde_json::from_str(&attribute_str){
                            Ok(result) =>result,
                            Err(err) => {
                        ...
                        for map in attributes {
                            for (name, attribute) in map{
                                ...
                                if let Some(color) = cell.get_style().get_background_color() {
                                    if let Some(source) = source_coding.get(&ColorString { argb: color.get_argb().to_string()}) {
                    ...
                    _ => {
                        ...
                        let (value, tol) = parse_tol_value(&value_str).expect(&format!("Error value str: {}, in property: {}", &value_str, &property.name));
...
mod tests {
    ...
    fn read_legend(){
        ...
        let recived_coding = SourceColorCoding::from_sheet(&sheet, &data.config.transformers.unwrap());
        let expected_color_coding: HashMap<ColorString, Source> =  HashMap::from([
            (ColorString { argb: String::from("FFFF0000") }, Source{source_type: SourceType::APPROXIMATION, remark: String::new()}),
            (ColorString { argb: String::from("FFFFFF00") }, Source{source_type: SourceType::LITERATURE, remark: String::new()}),
            (ColorString { argb: String::from("FF92D050") }, Source{source_type: SourceType::EXPERIMENT, remark: String::new()}),
            (ColorString { argb: String::from("FF00B0F0") }, Source{source_type: SourceType::COMPUTATION, remark: String::new()}),
        ]);
        assert_eq!(recived_coding.color_coding.len(), expected_color_coding.len());
        for (color_expected, source_expected) in &expected_color_coding{
            ...
            assert_eq!(source_found.unwrap().source_type, source_expected.source_type);
        }
    }
