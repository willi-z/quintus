...
fn data_fill_from_exel(sql: &mut SQLiteData) {
    ...
    let excel = ExcelData::new(
        resource!("excel/quintus_data_v1.0.0.xlsx"),
        resource!("excel/config_v1.0.0.json"),
    )
    .expect("EXCEL file or config was not found!");
    for component in excel.components().iter() {
        if sql.does_similar_exits(&component) {
            print!(".");
            continue;
        }
        sql.write_component(component, &WriteModus::Insert)
            .expect("Writing new component should not fail.");
    }
    println!("");
    sql.clean_up();
}
...
fn data_add_literature(sql: &mut SQLiteData) {
    ...
    let literature = vec![
        (
            "Asp et al.".to_owned(),
            "10.1002/aesr.202000093".to_owned(),
            24.0,
            25.0,
        ),
        (
            "Saraj et al.".to_owned(),
            "10.1002/aesr.202300109".to_owned(),
            41.0,
            26.0,
        ),
        (
            "Meng  et al.".to_owned(),
            "10.1021/acs.nanolett.8b03510".to_owned(),
            1.5,
            0.7,
        ),
        (
            "Thakur & Dong".to_owned(),
            "A. Thakur, X. Dong, Manuf. Lett. 2020, 24,1.".to_owned(),
            24.0,
            0.29,
        ),
        (
        ...
        (
            "Liu".to_owned(),
            "10.1016/j.jpowsour.2008.09.082".to_owned(),
            35.0,
            3.0,
        ),
        (
            "Comm. Pouchcell".to_owned(),
            "10.1002/aesr.202000093".to_owned(),
            240.0,
            0.7,
        ),
    ...
    for (authors, doi, energy_val, stiffness_val) in literature {
        ...
        energy.source = Some(Source {
            source_type: quintus::structures::SourceType::LITERATURE,
            remark: doi.clone(),
        });
        ...
        sql.write_component(&batt_lit, &WriteModus::Insert)
...
fn data_extension(sql: &mut SQLiteData) {
    ...
    let mut runner = runner::Evaluater::new(
        &read_from,
        &mut write_to,
        vec![filler::active_layers(), filler::capacity_evaluation()],
        None,
    )
    .unwrap();
    runner.evaluate_all();
...
fn data_evaluation(sql: &mut SQLiteData) {
    ...
    let mut runner = runner::Evaluater::new(
        ...
        &mut write_to,
        vec![
            battery::mass::areal_mass_evaluation(),
            battery::mass::electrolyte_mass_evaluation(),
            battery::stiffness::stiffness_evaluation(),
            battery::electric::capacity_evaluation(),
            battery::electric::voltage_evaluation(),
            battery::electric::energy_evaluation(),
        ],
