...
use plotters::{
    coord::Shift,
    prelude::{
        ChartBuilder, Circle, DrawingArea, DrawingBackend, FontStyle, PathElement, Polygon, Text, TextStyle, BLACK
    },
    style::{text_anchor::{HPos, Pos, VPos}, Color, FontDesc, FontFamily, FontTransform, IntoFont, IntoTextStyle, RGBAColor, ShapeStyle, WHITE},
};
    ...
    <DB as DrawingBackend>::ErrorType: 'static,
{
    ...
    if benchmarks.is_some() {
        ...
        let caption_style = TextStyle{font: caption_font, color: area_label_color.to_backend_color(), pos: Pos::new(HPos::Center, VPos::Center)};
        root_drawing_area.draw_text("Multifunktionali-", &caption_style, (width/2+20*scale, height/2))?;
        root_drawing_area.draw_text("tätsbereich", &caption_style, (width/2+20*scale, height/2 + 5*scale))?;
    }
    ...
    chart
        ...
        .x_label_style(
            ("serif", 4*scale).with_color(BLACK)
        )
        .y_label_style(
            ("serif", 4*scale).with_color(BLACK)
        )
    ...
    chart.draw_series(generated.iter().map(|(_, v)| {
        plotters::element::EmptyElement::at((v[0] as f32,v[1] as f32))
        +Circle::new((0, 0), 1*scale+1, ShapeStyle {
    ...
    .legend( |(x, y)| 
        ...
        + Circle::new((0, 0), 2*(scale-1), ShapeStyle {
            color: RGBAColor(119, 123, 132, 1.0),
            filled: true,
            stroke_width: 0,
        })
    ...
    let label_style =  TextStyle{font: label_font, color: BLACK.to_backend_color(), pos: Pos::new(HPos::Left, VPos::Top)};
    chart
    ...
    let caption_font = FontDesc::new(FontFamily::Serif, (6*scale).into(), FontStyle::Bold);
    let caption_style = TextStyle{font: caption_font, color: BLACK.to_backend_color(), pos: Pos::new(HPos::Center, VPos::Center)};
    root_drawing_area.draw_text(&x_label, &caption_style, (width/2 + 2*scale+1, height-2*scale -2))?;
    root_drawing_area.draw_text(&y_label, &caption_style.transform(FontTransform::Rotate270), (2*scale, height/2 - 3 * scale))?;
