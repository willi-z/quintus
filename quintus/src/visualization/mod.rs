...
use plotters::{
    coord::Shift,
    prelude::{
        ChartBuilder, Circle, DrawingArea, DrawingBackend, PathElement, Polygon, BLACK, RED,
    },
    style::{Color, RGBAColor, ShapeStyle, WHITE},
};
    ...
    <DB as DrawingBackend>::ErrorType: 'static,
{
    ...
    let property_names: Vec<String> = properties
        .iter()
        .map(|&(property_name, _unit)| property_name.to_string())
        .collect();
    ...
    chart.draw_series(literature.iter().enumerate().map(|(idx, (_, v))| {
        Circle::new(
            (v[0] as f32, v[1] as f32),
            4,
            ShapeStyle {
    ...
    chart.draw_series(literature.iter().enumerate().map(|(_idx, (_, v))| {
        Circle::new(
            (v[0] as f32, v[1] as f32),
            4,
            ShapeStyle {
