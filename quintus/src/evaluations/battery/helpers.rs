use crate::{evaluations::composer::{Composer, PropertyType}, structures::{Component, ComponentType}};


pub fn generate_layup_ids(composer: &Composer) -> Vec<ComponentType>{
    let outer_electrode = match composer.properties.get("OUTER_ELECTRODE_LAYER").expect("Composer for Battery should have Property: 'OUTER_ELECTRODE_LAYER'") {
        PropertyType::String(txt) => txt,
        _ => return Vec::new()
    };
    let num_electrode_layers = match composer.properties.get("NO_ELECTRODE_LAYERS").expect("Composer for Battery should have Property: 'NO_ELECTRODE_LAYERS'"){
...
pub fn generate_layup<'a>(composer: &Composer, anode: &'a Component, cathode: &'a Component, foil: &'a Component, separator: &'a  Component) -> Vec<&'a Component>{
...
pub fn no_of_cells_in_stack(composer: &Composer, anode_no_active_layers: u8, cathode_no_active_layers: u8) -> u32{
    ...
    while layup[index_layer] != String::from("anode") && layup[index_layer] != String::from("cathode") {
        index_layer += 1
    }
    for idx_electrode in 0..num_electrode_layers -1{ // ignore last layer
        let mut index_layer_next_electrode = index_layer +1;
        while layup[index_layer_next_electrode] != String::from("anode") && layup[index_layer_next_electrode] != String::from("cathode") {
        ...
        if usages[idx_electrode] > match layup[index_layer].as_str() {"anode" => anode_no_active_layers, "cathode" => cathode_no_active_layers, _=> 0}{
            usages[idx_electrode] -= 1;
            usages[idx_electrode+1] -= 1;
            num_cells -= 1;
        }
...
mod tests {
    ...
    fn test_layup(){
        let test_sets = vec![
            (2, Electrode::Anode, vec!["foil", "anode", "separator", "cathode", "foil"]),
            (3, Electrode::Cathode, vec!["foil", "cathode", "separator", "anode", "separator", "cathode", "foil"]),
            (4, Electrode::Cathode, vec!["foil", "cathode", "separator", "anode", "separator", "cathode", "separator", "anode", "foil"]),
            (5, Electrode::Anode, vec!["foil", "anode", "separator", "cathode", "separator", "anode", "separator", "cathode", "separator", "anode", "foil"]),
        ];
    ...
    fn test_no_of_cells_in_stack(){
        ...
        for (el_layers, outer_electrode,anode_alayers, cathode_alayers, expected_cells) in test_sets{
            let composer = new_battery_composer(el_layers, outer_electrode);
