...
pub(crate) fn electrolyte_mass_func(composer: &Composer, components: &HashMap<ComponentType, &Component>) -> f64 {
    let electrolyte = *components.get("electrolyte").unwrap();
    return electrolyte.properties.get("electrolyte_mass_per_capacity").unwrap().get_value_in_si() * capacity_func(composer, components);
}

pub fn electrolyte_mass_evaluation() -> Evaluation {
    Evaluation::new_require_exisiting_property_only(
        "areal_electrolyte_mass",
        Unit{ unit: String::from("kg/m2"), to_si_factor: 1000.0 },
        HashMap::from([
...
pub(crate) fn mass_func(composer: &Composer, components: &HashMap<ComponentType, &Component>) -> f64 {
    ...
    for component in layup{
        m_sum += component.properties.get("areal_mass").unwrap().get_value_in_si();
    }
...
pub fn areal_mass_evaluation() -> Evaluation {
    Evaluation::new_require_exisiting_property_only(
        ...
        Unit{ unit: String::from("kg/m2"), to_si_factor: 1000.0 },
        HashMap::from([
            ("anode".to_string(), vec!["areal_mass".to_string(), "areal_capacity".to_string(), "active_layers".to_string()]),
            ("cathode".to_string(), vec!["areal_mass".to_string(), "areal_capacity".to_string(), "active_layers".to_string()]),
