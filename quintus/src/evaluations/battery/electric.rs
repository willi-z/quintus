...
pub fn capacity_func(composer: &Composer, components: &HashMap<ComponentType, &Component>) -> f64 {
    ...
    let anode_areal_capacity = anode.properties.get("areal_capacity").expect("Filter should have prevented not finding the measurement.");
    let anode_maximum_usage = anode.properties.get("active_layers").expect("Filter should have prevented not finding the measurement.").get_value_in_si();
    
    let cathode = *components.get("cathode").expect("Filter should have prevented not finding the component.");
    let cathode_areal_capacity = cathode.properties.get("areal_capacity").expect("Filter should have prevented not finding the measurement.");
    let cathode_maximum_usage = cathode.properties.get("active_layers").expect("Filter should have prevented not finding the measurement.").get_value_in_si();
    
    let eff_capacity = anode_areal_capacity.get_value_in_si().min(cathode_areal_capacity.get_value_in_si());
    let num_cells = no_of_cells_in_stack(composer, anode_maximum_usage as u8, cathode_maximum_usage as u8);
...
pub fn capacity_evaluation() -> Evaluation {
    Evaluation::new_require_exisiting_property_only(
        "areal_capacity",
        Unit{ unit: String::from("C/m2"), to_si_factor: 1.0 },
        HashMap::from([
            ("anode".to_string(), vec!["areal_capacity".to_string(), "active_layers".to_string()]),
            ("cathode".to_string(), vec!["areal_capacity".to_string(), "active_layers".to_string()]),
        ]),
...
fn voltage_func(_composer: &Composer, components: &HashMap<ComponentType, &Component>) -> f64 {
    ...
    let anode_potential = anode.properties.get("potential_vs_li").expect("Filter should have prevented not finding the measurement.");
    let cathode_potential = cathode.properties.get("potential_vs_li").expect("Filter should have prevented not finding the measurement.");
...
pub fn voltage_evaluation() -> Evaluation {
    Evaluation::new_require_exisiting_property_only(
        "voltage",
        Unit{ unit: String::from("V"), to_si_factor: 1000.0 },
        HashMap::from([
...
pub fn energy_evaluation() -> Evaluation {
    Evaluation::new_require_exisiting_property_only(
        "energy_density",
        Unit{ unit: String::from("Wh/kg"), to_si_factor: 3600.0 },
        HashMap::from([
            ("anode".to_string(), vec!["areal_mass".to_string(), "areal_capacity".to_string(), "active_layers".to_string(), "potential_vs_li".to_string()]),
            ("cathode".to_string(), vec!["areal_mass".to_string(), "areal_capacity".to_string(), "active_layers".to_string(), "potential_vs_li".to_string()]),
            ("separator".to_string(), vec!["areal_mass".to_string()]),
            ("foil".to_string(), vec!["areal_mass".to_string()]),
            ("electrolyte".to_string(), vec!["electrolyte_mass_per_capacity".to_string()])
        ]),
        Box::new(energy_func)
    )
}
...
#[cfg(test)]
mod tests {
    use crate::{evaluations::battery::{new_battery_composer, Electrode}, structures::Measurement};
    ...
    fn test_capacity_func(){
        ...
        assert_eq!(capacity_func(&composer, &components), 1.0 * unit.to_si_factor );

        let eval = capacity_evaluation();
        assert_eq!(eval.evaluate(&composer, &components).get_value_in_si(), 1.0 * unit.to_si_factor );
    ...
    fn test_voltage_func(){
        ...
        let result = voltage_func(&composer, &HashMap::from([
            ("anode".to_string(), &anode),
            ("cathode".to_string(), &cathode)
        ]))/ unit.to_si_factor;
