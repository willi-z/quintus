use crate::{
    helpers::queries::select_component_with_properties,
    structures::{Component, ComponentType, Measurement, PropertyName, Source, SourceType, Unit},
};
...
impl Evaluation {
    ...
    pub fn new_require_exisiting_property_only(
        ...
        for (ctype, property_names) in required_properties.iter() {
            components_filter.insert(
                ctype.clone(),
                select_component_with_properties(property_names),
            );
        }
        Evaluation {
            result_name: result_name.to_string(),
    ...
    pub fn evaluate(
        &self,
        composer: &Composer,
        input: &HashMap<ComponentType, &Component>,
    ) -> Measurement {
