...
fn capacity_func(_composer: &Composer, components: &HashMap<ComponentType, &Component>) -> f64 {
    let active_layer = *components.get("active layer").expect("Filter should have prevented not finding the component.");
    let areal_capacity = active_layer.properties.get("areal_capacity").expect("Filter should have prevented not finding the measurement.");
...
pub fn capacity_evaluation() -> Evaluation {
    Evaluation::new_require_exisiting_property_only(
        ...
        HashMap::from([
            ("active layer".to_string(), vec!["areal_capacity".to_string()]),
        ]),
...
fn active_layers_func(_composer: &Composer, components: &HashMap<ComponentType, &Component>) -> f64 {
    let active_layer = *components.get("active layer").expect("Filter should have prevented not finding the component.");
    let layers = active_layer.properties.get("layers").expect("Filter should have prevented not finding the measurement.");
...
pub fn active_layers() -> Evaluation {
    Evaluation::new_require_exisiting_property_only(
        "active_layers",
        Unit{ unit: String::from(""), to_si_factor: 1.0 },
        HashMap::from([
            ("active layer".to_string(), vec!["layers".to_string()]),
        ]),
...
mod tests {
    ...
    fn test_capacity_func(){
        ...
        let components = HashMap::from([
            ("active layer".to_string(), &electrode),
        ]);
        assert_eq!(capacity_func(&composer, &components), 2.3 * unit_old.to_si_factor );

        let eval = capacity_evaluation();
        assert_eq!(eval.evaluate(&composer, &components).get_value_in_si(), 2.3 * unit_old.to_si_factor );
    }
}